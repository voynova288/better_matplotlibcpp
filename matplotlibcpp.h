...
private:
  ...
  _interpreter() {
    ...
    //*在python3.11版本，下面的那行代码中的Py_SetProgramName函数已经被弃用了
    //*下面这段代码是gpt给我的修改方案
    //*据说直接注释掉Py_SetProgramName(name);大多数情况下不会出问题
    // #if PY_VERSION_HEX >= 0x030B0000  // 3.11 及更新版本
    //     PyConfig config;
    ...
    //*在conda的base环境下下载包后matplotlibcpp总是找不到numpy和matplotlib的引用路径
    //*下面这段代码手动添加引用路径
    PyRun_SimpleString("import sys");
    PyRun_SimpleString(
        "sys.path.append('/home/voynova/miniconda3/lib/python3.12/"
        "site-packages')");
...
#if PY_MAJOR_VERSION >= 3
    // #if PY_VERSION_HEX >= 0x030B0000  //*python版本大于等于3.11
    //     //*在python3.11及以上版本，PySys_SetArgv被弃用，下面这段代码是gpt给我的解决方案
    //     PyConfig_SetArgv(&config, argc, const_cast<wchar_t* const*>(argv));
    // #else
...
void plot_surface(const std::vector<::std::vector<Numeric>> &x,
  ...
  // 获取 fig.add_subplot 方法
  PyObject *add_subplot = PyObject_GetAttrString(fig, "add_subplot");
  if (!add_subplot)
    throw std::runtime_error("No add_subplot");

  // 构建位置参数，例如 111 表示 1 行 1 列的第 1 个子图
  PyObject *args_surface = Py_BuildValue("(i)", 111);
  if (!args)
    throw std::runtime_error("Failed to build args");

  // 构建关键字参数，projection 设置为 "3d"
  PyObject *kwargs_surface = Py_BuildValue("{s:s}", "projection", "3d");
  if (!kwargs)
    throw std::runtime_error("Failed to build kwargs");

  // 调用 add_subplot 创建 3D 坐标轴
  PyObject *axis = PyObject_Call(add_subplot, args_surface, kwargs_surface);
  if (!axis)
    throw std::runtime_error("No axis");

  Py_DECREF(add_subplot);
  Py_DECREF(args_surface);
  Py_DECREF(kwargs_surface);
...
bool contourf(const std::vector<NumericX> &x, const std::vector<NumericY> &y,
  ...
  PyObject *res = PyObject_Call(
      detail::_interpreter::get().s_python_function_contourf, plot_args, kwargs);
